...
export function pdlBlockedReason(sc) {
  ...
  if (paid / agreed < MIN_PAYMENT_RATIO_FOR_PDL)
    return `Pay at least ${Math.round(MIN_PAYMENT_RATIO_FOR_PDL * 100)}% to add PDL`;
  return null;
}
...
export function canSubmitForExam(sc) {
  ...
  if (sc.status === "active") {
    return balance <= 0 && sc.pdl_state === "active";
  }
  return false;
}
...
export function adminPrimaryAction(student) {
  ...
  if (examCourse) return { kind: "approve-exam", label: "Approve Exam", resourceId: examCourse.exam_booking.id };
...
export function branchPrimaryAction(student) {
  ...
  if (courses.some((sc) => sc.status === "pending_pdl"))
    return { kind: "submit-pdl", label: "Add PDL" };
